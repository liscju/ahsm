...
public class SiteContentMerger {
    ...
    private Element firstFullEntry(Element entry) {
        ...
        for (ListIterator<Element> iterator = table.children().listIterator(); iterator
                .hasNext();) {
            total1++;
    ...
    private Element mergeEntry(Element entry1, Element entry2, String key) {
        ...
        // first check
        for (ListIterator<Element> iterator1 = table1.children().listIterator(1); iterator1
                .hasNext();) {
            final Element current1 = iterator1.next();
            boolean unique1 = true;
            for (ListIterator<Element> iterator2 = table2.children().listIterator(); iterator2
                    .hasNext();) {
        ...
        // second check
        for (ListIterator<Element> iterator2 = table2.children().listIterator(1); iterator2
                .hasNext();) {
            final Element current2 = iterator2.next();
            boolean unique2 = true;
            for (ListIterator<Element> iterator1 = table1.children().listIterator(); iterator1
                    .hasNext();) {
                final Element current1 = iterator1.next();
                if (rowEqualityCheck(current1, current2)) {
                    unique2 = false;
    ...
    private Element generateSummary() {
        ...
        Element mergedSummaryTable = content1.getSummarySection().getElementsByTag("table").first()
                .clone();
        ...
        Element summary2ResultRow = content2.getSummarySection().getElementsByTag("tr").get(1)
                .clone();
